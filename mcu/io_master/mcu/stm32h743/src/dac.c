...
//    HAL_StatusTypeDef ret;
    ...
    //Set VRef
    HAL_I2C_Master_Transmit(&hi2c3, EXT_DAC_ADDR, (uint8_t*) &bufVRef, 1, HAL_MAX_DELAY);
    //errorLight(ret);
    ...
    //Update Voltages
    HAL_I2C_Master_Transmit(&hi2c3, EXT_DAC_ADDR_UPDATE, (uint8_t*) &bufUpdate, 1, HAL_MAX_DELAY);
    //errorLight(ret);
    return IOM_OK;
  }
  //TESTING
  ...
  IOM_ERROR WriteRegExtDAC(IOM_REGISTER reg, uint8_t channel, uint16_t vOut)
  {
    ...
    //Write Channel Registers
    if(HAL_I2C_Master_Transmit(&hi2c3, EXT_DAC_ADDR, buf, 8, HAL_MAX_DELAY) != HAL_OK) {
      return IOM_ERROR_INTERFACE;
    }

    //Update Voltages
    if(HAL_I2C_Master_Transmit(&hi2c3, EXT_DAC_ADDR_UPDATE, (uint8_t*)&bufUpdate, 1, HAL_MAX_DELAY) != HAL_OK) {
      return IOM_ERROR_INTERFACE;
    }
    return IOM_OK;
  }
