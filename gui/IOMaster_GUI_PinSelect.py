...
def Configure_Results():
    ...
    if Protocol == "Custom Packet":
        ...
        byteSend(rewr, commandbits, resisterStates, byte2, byte3, byte4, byte5.rstrip('\n'))
        return
...
def byteSend(rewr, commandbits, resisterStates, byte2, byte3, byte4, byte5):
    #rewr=1 bit, commandbits=3, resisterstate=4 bits
    rewr = rewr << 7
    combits = commandbits << 4
...
def pinConfiguration():
    ...
    byteSend(rewr,commandbits,resisterStates,pinPattern,'NULL','NULL','NULL')
